...
/**
 * Secondary authorization endpoint MoP returns to after the upstream Datadog OAuth code flow
 * completes. Mirrors {@link FigmaResource}: Quarkus OIDC owns the
 * {@code /datadog/authorize/callback} leg (configured via
 * {@code quarkus.oidc.datadog.authentication.redirect-path}), then with
 * {@code restore-path-after-redirect: true} it bounces the browser back to
 * {@code /datadog/authorize?state=&lt;mopAuthCode&gt;} -- this method.
 *
 * <p>Datadog access tokens last 1 hour, well under the global {@code server.token-store.expiry}
 * default (~8h). The 7-arg {@code AuthorizerService.storeTokens} overload is therefore enough;
 * unlike Figma we do not need to pin a longer L1 row TTL.
 *
 * <p>Datadog DCR returns no client_secret (public PKCE client, {@code token_endpoint_auth_method=none}),
 * so the {@code datadog} OIDC tenant has no {@code credentials} block. PKCE alone is the upstream
 * authentication mechanism.
 */
@Path("/datadog/authorize")
public class DatadogResource extends BaseResource {

    private static final Logger log = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());

    static final String PROVIDER = "datadog";
    ...
    public Response authorize(@QueryParam("state") String state) {
        ...
        if (authorizationCode == null) {
            log.warn("Datadog callback: authorization code not found for state (local or cross-region)");
            return Response.status(Response.Status.BAD_REQUEST)
                    .entity(Map.of(
                            "error", "invalid_grant",
        ...
        log.info("Datadog request to store tokens for user: {}", lookupKey);
        ...
        if (refreshToStore == null || refreshToStore.isEmpty()) {
            ...
            // (rotation-aware, sibling-aware) when this Datadog callback has no fresh RT in the
            // OIDC session -- covers second/third MCP-client windows relogging into an
            // existing Datadog upstream session.
