...
/**
 * Secondary authorization endpoint MoP returns to after the upstream Airtable OAuth code flow
 * completes. Mirrors {@link FigmaResource}: Quarkus OIDC owns the
 * {@code /airtable/authorize/callback} leg (configured via
 * {@code quarkus.oidc.airtable.authentication.redirect-path}), then with
 * {@code restore-path-after-redirect: true} it bounces the browser back to
 * {@code /airtable/authorize?state=<mopAuthCode>} -- this method.
 *
 * <p>Airtable is a confidential PKCE client using HTTP Basic on the token endpoint. Access tokens
 * are short-lived (1 h, {@code expires_in=3600}) and the refresh token <strong>rotates</strong>
 * on every successful refresh (RT lifetime 60 d). The 8-arg
 * {@link AuthorizerService#storeTokens(String, String, String, String, String, String, String, Long)}
 * overload is used so the bare {@code (lookupKey, "airtable")} row in
 * {@code mcp-oauth-proxy-tokens} is provider-pinned to the documented access-token lifetime even
 * though 1 h is already under the global ~8 h {@code server.token-store.expiry}; this matches the
 * Figma/Linear/Wisdom/Oracle-EPM precedent and keeps row TTL behavior provider-explicit.
 */
@Path("/airtable/authorize")
public class AirtableResource extends BaseResource {

    private static final Logger log = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());

    static final String PROVIDER = "airtable";

    /** Airtable's documented access-token lifetime (1 h). Pinned so the L1 bare row TTL is provider-explicit. */
    static final long AIRTABLE_ACCESS_TOKEN_LIFETIME_SECONDS = 3_600L;
    ...
    public Response authorize(@QueryParam("state") String state) {
        ...
        if (authorizationCode == null) {
            log.warn("Airtable callback: authorization code not found for state (local or cross-region)");
            return Response.status(Response.Status.BAD_REQUEST)
                    .entity(Map.of(
                            "error", "invalid_grant",
        ...
        log.info("Airtable request to store tokens for user: {}", lookupKey);
        ...
        if (refreshToStore == null || refreshToStore.isEmpty()) {
            ...
            // (rotation-aware, sibling-aware) when this Airtable callback has no fresh RT in the
            // OIDC session -- covers second/third MCP-client windows relogging into an
            // existing Airtable upstream session.
        ...
        authorizerService.storeTokens(
                ...
                AIRTABLE_ACCESS_TOKEN_LIFETIME_SECONDS);
