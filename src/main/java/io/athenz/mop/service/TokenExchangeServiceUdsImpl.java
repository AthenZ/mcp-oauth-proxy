...
/**
 * Token "exchange" for the UDS MCP: there is no exchange at all. Like
 * {@link TokenExchangeServiceYahooSynapseImpl}, the UDS MCP server accepts the raw Okta
 * {@code access_token} as its bearer. This impl simply surfaces the Okta {@code access_token} as
 ...
 *
 * <p>UDS is Okta-rooted and has no upstream refresh token of its own: refresh is performed via the
 * shared Okta path (the single L2 {@code okta#<sub>} row under {@link UpstreamRefreshService}),
 * exactly like Glean / Evaluate / Yahoo OS / Yahoo Synapse. Accordingly
 ...
 */
@ApplicationScoped
public class TokenExchangeServiceUdsImpl implements TokenExchangeService {
    ...
    public AuthorizationResultDO getJWTAuthorizationGrantFromIdentityProvider(TokenExchangeDO tokenExchangeDO) {
        throw new UnsupportedOperationException(
                "UDS exchange uses getAccessTokenFromResourceAuthorizationServer");
    }

    @Override
    public AuthorizationResultDO getAccessTokenFromResourceAuthorizationServer(TokenExchangeDO tokenExchangeDO) {
        String oauthProvider = OauthProviderLabel.UDS;
        ...
        if (oktaToken == null || StringUtils.isBlank(oktaToken.accessToken())) {
            log.warn("UDS exchange: missing Okta access_token");
            recordStep(oauthProvider, oauthClient, region, t0, false);
            return AuthorizationResultDO.unauthorized("UDS exchange: missing Okta access_token");
        }
    ...
    public AuthorizationResultDO getAccessTokenFromResourceAuthorizationServerWithClientCredentials(
        ...
        throw new UnsupportedOperationException("UDS exchange does not support client credentials");
    }
    ...
    public TokenWrapper refreshWithUpstreamToken(String upstreamRefreshToken) {
        ...
        // Glean / Evaluate / Yahoo OS / Yahoo Synapse / GCP providers).
        return null;
    }
    ...
    private void recordStep(String oauthProvider, String oauthClient, String region,
        ...
        oauthProxyMetrics.recordExchangeStep(ExchangeStep.UDS_ACCESS_TOKEN, oauthProvider, success,
