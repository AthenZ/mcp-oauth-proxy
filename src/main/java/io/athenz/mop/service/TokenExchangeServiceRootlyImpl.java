...
/**
 * Rootly OAuth integration. Rootly is a plain OAuth consent-passthrough provider: a confidential
 * client ({@code client_secret_post}) + PKCE whose token endpoint is
 * {@code https://rootly.com/oauth/token}. MoP stores the upstream access + refresh tokens and the
 * downstream token flow returns the access token unchanged (PASS_THROUGH — no token exchange).
 *
 * <p>Note: with Rootly promoted to the L2 model in
 * {@link UpstreamProviderClassifier#isUpstreamPromoted(String)}, the canonical refresh path is
 * {@code UpstreamRefreshService.refreshUpstream("rootly", ...)} → {@link RootlyUpstreamRefreshClient}.
 * {@link #refreshWithUpstreamToken(String)} below remains for the legacy/native fallback path used
 * by {@code AuthorizerService.refreshUpstreamAndGetToken}.
 */
@ApplicationScoped
public class TokenExchangeServiceRootlyImpl implements TokenExchangeService {

    private static final Logger log = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());
    private static final URI ROOTLY_TOKEN_ENDPOINT = URI.create("https://rootly.com/oauth/token");
    ...
    public TokenWrapper refreshWithUpstreamToken(String upstreamRefreshToken) {
        ...
        if (clientId == null || clientId.isBlank()) {
            log.warn("Rootly refresh: client_id not configured");
            return null;
        }

        if (clientSecretKey == null || clientSecretKey.isBlank()) {
            log.warn("Rootly refresh: client secret key not configured (server.token-exchange.rootly.client-secret-key)");
            return null;
        }
        ...
        if (clientSecret == null || clientSecret.isBlank()) {
            log.warn("Rootly refresh: client secret missing (key={})", clientSecretKey);
            return null;
        }

        long t0 = System.nanoTime();
        String oauthProvider = OauthProviderLabel.ROOTLY;
        ...
        try {
            ...
            if (tokenResponse.indicatesSuccess()) {
                ...
                // Rely on Rootly's reported expires_in verbatim; never pin a fixed AT lifetime.
                // sanitizeExpiresIn() downstream coerces a null/0 ttl to the generic default.
                long ttl = (lifetime != null && lifetime > 0) ? lifetime : 0L;
                recordRootlyRefresh(t0, oauthProvider, oauthClient, region, true);
                ...
                log.error("Rootly refresh failed; upstream response: {}", UpstreamTokenRefreshErrors.formatTokenError(errorResponse));
                recordRootlyRefresh(t0, oauthProvider, oauthClient, region, false);
                return null;
            }
        } catch (Exception e) {
            log.error("Rootly refresh failed (could not complete token request or parse upstream response)", e);
            recordRootlyRefresh(t0, oauthProvider, oauthClient, region, false);
            return null;
        }
    }

    private void recordRootlyRefresh(long startNanos, String oauthProvider, String oauthClient, String region, boolean success) {
