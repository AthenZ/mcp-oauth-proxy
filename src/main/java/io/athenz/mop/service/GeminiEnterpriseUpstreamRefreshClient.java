...
/**
 * {@link UpstreamRefreshClient} for the Gemini Enterprise (GE Stream Assist) provider.
 *
 * <p>Gemini Enterprise authenticates the user via direct Google OAuth/OIDC, exactly like the
 * Google Workspace providers, but it uses its <strong>own dedicated Google OAuth client</strong>
 * (separate {@code client_id} and {@code gemini-enterprise-client-secret}) instead of the shared
 * MoP Google Workspace web-app client. The refresh wire shape is identical to
 * {@link GoogleWorkspaceUpstreamRefreshClient}: {@code grant_type=refresh_token} against
 * {@code https://oauth2.googleapis.com/token} with {@code client_secret_post} auth.
 *
 * <p>Error contract:
 * <ul>
 ...
 */
@ApplicationScoped
public class GeminiEnterpriseUpstreamRefreshClient implements UpstreamRefreshClient {

    private static final Logger log = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());
    private static final String GOOGLE_TOKEN_URI = "https://oauth2.googleapis.com/token";
    private static final long DEFAULT_EXPIRES_IN_SECONDS = 3600L;

    @ConfigProperty(name = "server.token-exchange.gemini-enterprise.client-id", defaultValue = "")
    String clientId;

    @ConfigProperty(name = "server.token-exchange.gemini-enterprise.client-secret-key",
            defaultValue = "gemini-enterprise-client-secret")
    ...
    public UpstreamRefreshResponse refresh(String providerUserId, String upstreamRefreshToken) {
        if (upstreamRefreshToken == null || upstreamRefreshToken.isBlank()) {
            throw new OktaTokenRefreshException("Gemini Enterprise upstream refresh token is empty");
        }
        if (clientId == null || clientId.isBlank()) {
            throw new OktaTokenRefreshException(
                    "Gemini Enterprise client_id not configured (server.token-exchange.gemini-enterprise.client-id)");
        }
        if (clientSecretKey == null || clientSecretKey.isBlank()) {
            throw new OktaTokenRefreshException(
                    "Gemini Enterprise client secret key not configured "
                            + "(server.token-exchange.gemini-enterprise.client-secret-key)");
        }
        ...
        if (clientSecret == null || clientSecret.isEmpty()) {
            throw new OktaTokenRefreshException(
                    "Gemini Enterprise client secret not found (key=" + clientSecretKey + ")");
        }
        ...
        try {
            ...
            try (var ignored = UpstreamHttpCallLabels.withLabels(
                    OauthProviderLabel.GEMINI_ENTERPRISE, UpstreamHttpCallLabels.ENDPOINT_OAUTH_TOKEN)) {
            ...
            log.error("Gemini Enterprise refresh failed for provider_user_id={} upstream response: {}",
                    providerUserId, body);
            ...
            if ("invalid_grant".equals(code)) {
                throw new OktaTokenRevokedException("Gemini Enterprise refresh token invalid or revoked: " + desc);
            }
            throw new OktaTokenRefreshException("Gemini Enterprise token refresh failed: " + code + " - " + desc);
        } catch (OktaTokenRevokedException | OktaTokenRefreshException e) {
            throw e;
        } catch (Exception e) {
            log.error("Gemini Enterprise refresh failed for provider_user_id={} "
                    + "(could not complete token request or parse upstream response)", providerUserId, e);
            throw new OktaTokenRefreshException("Gemini Enterprise token refresh failed: " + e.getMessage(), e);
        }
    }
}
