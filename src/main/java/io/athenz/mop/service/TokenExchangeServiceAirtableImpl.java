...
/**
 * Airtable OAuth integration. Access tokens last 1 hour ({@code expires_in=3600}); the
 * {@code POST} body is {@code application/x-www-form-urlencoded} with {@code client_secret_basic}
 * authentication (matches Airtable's documented curl example using {@code -u id:secret}).
 *
 * <p>Note: with Airtable promoted to the L2 model in
 * {@link UpstreamProviderClassifier#isUpstreamPromoted(String)}, the canonical refresh path is
 * {@code UpstreamRefreshService.refreshUpstream("airtable", ...)} -&gt;
 * {@link AirtableUpstreamRefreshClient}. {@link #refreshWithUpstreamToken(String)} below
 * remains for the legacy/native fallback path used by
 * {@code AuthorizerService.refreshUpstreamAndGetToken}.
 *
 * <p>Airtable <strong>rotates</strong> the refresh token on each refresh (RT lifetime 60 d); we
 * always persist the response RT verbatim. Defensive carry-forward of the prior RT is retained
 * only for the (anomalous) case where the response is HTTP 200 but contains no new
 * refresh_token.
 */
@ApplicationScoped
public class TokenExchangeServiceAirtableImpl implements TokenExchangeService {

    private static final Logger log = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());
    private static final URI AIRTABLE_TOKEN_ENDPOINT = URI.create(
            "https://airtable.com/oauth2/v1/token");
    /** Airtable documents 3,600 s (1 h) for access tokens; used only when the response omits {@code expires_in}. */
    static final long AIRTABLE_DEFAULT_TOKEN_TTL = 3_600L;
    ...
    public TokenWrapper refreshWithUpstreamToken(String upstreamRefreshToken) {
        ...
        if (clientId == null || clientId.isBlank()) {
            log.warn("Airtable refresh: client_id not configured");
            return null;
        }

        if (clientSecretKey == null || clientSecretKey.isBlank()) {
            log.warn("Airtable refresh: client secret key not configured (server.token-exchange.airtable.client-secret-key)");
            return null;
        }
        ...
        if (clientSecret == null || clientSecret.isBlank()) {
            log.warn("Airtable refresh: client secret missing (key={})", clientSecretKey);
            return null;
        }

        long t0 = System.nanoTime();
        String oauthProvider = OauthProviderLabel.AIRTABLE;
        ...
        try {
            ...
            // Confidential client (client_secret_basic): client_id and client_secret are sent on
            // the HTTP `Authorization: Basic <base64(id:secret)>` header (Airtable's verified
            // curl shape using `-u id:secret`). Do NOT switch to ClientSecretPost — Airtable's
            // documented examples and the working flow both use HTTP Basic.
            RefreshTokenGrant grant = new RefreshTokenGrant(new RefreshToken(refreshTokenValue));
            TokenRequest tokenRequest = new TokenRequest(
                    AIRTABLE_TOKEN_ENDPOINT,
            ...
            if (tokenResponse.indicatesSuccess()) {
                ...
                long ttl = (lifetime != null && lifetime > 0) ? lifetime : AIRTABLE_DEFAULT_TOKEN_TTL;
                recordAirtableRefresh(t0, oauthProvider, oauthClient, region, true);
                return new TokenWrapper(
                        null,
                        null,
                        null,
                        accessToken.getValue(),
                        // Airtable rotates the RT each refresh — persist the response RT verbatim
                ...
                log.error("Airtable refresh failed; upstream response: {}",
                        UpstreamTokenRefreshErrors.formatTokenError(errorResponse));
                recordAirtableRefresh(t0, oauthProvider, oauthClient, region, false);
                return null;
            }
        } catch (Exception e) {
            log.error("Airtable refresh failed (could not complete token request or parse upstream response)", e);
            recordAirtableRefresh(t0, oauthProvider, oauthClient, region, false);
            return null;
        }
    }

    private void recordAirtableRefresh(long startNanos, String oauthProvider, String oauthClient,
